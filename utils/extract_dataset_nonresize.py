...
store_file = '../datasets/data_new.h5'
train_size = 0.8
...
def extract_dataset():
    ...
    store_gt_loc_fix = np.zeros(shape=(0, image_size, image_size, 1))
    store_gt_glob_fix = np.zeros(shape=(0, image_size, image_size, 1))
    ...
    for i, image in enumerate(images):
        ...
        gt_glob_image, scaled_gt_glob_image, gt_glob_binary_fix = get_global_gt(name, blank_image, max_fix=4, sigma=sigma, kernel=kernel,
                                                            binary=False)
        if verbose == 1:
        ...
        for j, cnt in enumerate(contours):
            ...
            shape_image, scaled_shape_image, _ = get_clipped_shape(thr, gt_glob_binary_fix, bx, by, bw, bh, area, image_size, True)
            gt_loc_shape_image, scaled_gt_loc_shape_image, gt_loc_binary_fix = get_local_gt(name, blank_image, bx, by, bw, bh, max_fix=4, sigma=sigma,
            ...
            for model in loc_models:
                mod_img = cv.imread(os.path.join('../images/models/local/.', model, image), 0)
                mod_shape, scaled_mod_shape, _ = get_clipped_shape(mod_img, gt_glob_binary_fix, bx, by, bw, bh, area, image_size, True)
            ...
            gt_loc_binary_fix = gt_loc_binary_fix.reshape((1,) + gt_loc_binary_fix.shape)
            store_gt_loc_fix = np.append(store_gt_loc_fix, gt_loc_binary_fix, axis=0)

            #### GLOBAL CONTEXT ####

            gt_glob_shape_image, scaled_gt_glob_shape_image, gt_glob_fix_image = get_clipped_shape(gt_glob_image, gt_glob_binary_fix, bx, by, bw, bh, area, image_size, True)
            result_image = scaled_shape_image
            if verbose == 1:
            ...
            for model in glob_models:
                mod_img = cv.imread(os.path.join('../images/models/global/.', model, image), 0)
                mod_shape, scaled_mod_shape, _ = get_clipped_shape(mod_img, gt_glob_binary_fix, bx, by, bw, bh, area, image_size, True)
                # Create multichannel image
                result_image = np.concatenate((result_image, scaled_mod_shape), axis=2)
                if verbose == 1:
            ...
            # store_gt_glob.append(gt_glob_fix_image)
            gt_glob_fix_image = gt_glob_fix_image.reshape((1,) + gt_glob_fix_image.shape)
            store_gt_glob_fix = np.append(store_gt_glob_fix, gt_glob_fix_image, axis=0)
    ...
    valid_gt_loc = h5f.create_dataset("valid_gt_loc", (len(store_image_loc) - end_t, image_size, image_size, 1),
                                      maxshape=(None, image_size, image_size, 1), chunks=True)
    train_binary_gt_loc = h5f.create_dataset("train_binary_gt_loc", (end_t, image_size, image_size, 1),
                                      maxshape=(None, image_size, image_size, 1), chunks=True)
    valid_binary_gt_loc = h5f.create_dataset("valid_binary_gt_loc", (len(store_image_loc) - end_t, image_size, image_size, 1),
                                             maxshape=(None, image_size, image_size, 1), chunks=True)
    ...
    valid_gt_glob = h5f.create_dataset("valid_gt_glob", (len(store_image_loc) - end_t, image_size, image_size, 1),
                                       maxshape=(None, image_size, image_size, 1), chunks=True)
    train_binary_gt_glob = h5f.create_dataset("train_binary_gt_glob", (end_t, image_size, image_size, 1),
                                       maxshape=(None, image_size, image_size, 1), chunks=True)
    valid_binary_gt_glob = h5f.create_dataset("valid_binary_gt_glob", (len(store_image_loc) - end_t, image_size, image_size, 1),
                                              maxshape=(None, image_size, image_size, 1), chunks=True)
    ...
    train_gt_fix_set = store_gt_loc_fix[0:end_t, :, :, :]
    valid_gt_fix_set = store_gt_loc_fix[end_t:, :, :, :]
    ...
    train_binary_gt_loc[-len(train_set):] = train_gt_fix_set
    valid_binary_gt_loc[-len(valid_set):] = valid_gt_fix_set
    ...
    train_gt_fix_set = store_gt_glob_fix[0:end_t, :, :, :]
    valid_gt_fix_set = store_gt_glob_fix[end_t:, :, :, :]
    ...
    train_binary_gt_glob[-len(train_set):] = train_gt_fix_set
    valid_binary_gt_glob[-len(valid_set):] = valid_gt_fix_set
